    ...
    <div>
      <v-card elevation="10" class="mt-3">
          ...
          <v-data-table
              ...
              class="elevation-1"
          >
            <template slot="item.telefono" slot-scope="props">
              {{ props.item.telefono ? props.item.telefono : '-' }}
            </template>
            <template slot="item.direccion" slot-scope="props">
              {{ props.item.direccion ? props.item.direccion : '-' }}
            </template>
            <!--Contador Index-->
      ...
      <cModalComensal :num="modal.num" :payload="modal.payload" :title="modal.title" 
...
import cModalComensal from '@/components/Modals/cModalComensal.vue';
  export default {
    components: {
        SweetAlert,
        cModalComensal
    ...
    methods: {
      ...
      mtdModalShow(num, payload) {
        this.modal.title = (num == 0) ? 'Registrar Comensal' : 'Editar Comensal';
      ...
      async addDatadetail(payload_detail, num_detail) {
        try {
          const apiUrl = process.env.VUE_APP_API_URL;
          const url = `${apiUrl}/Comensal/crudComensal`;
